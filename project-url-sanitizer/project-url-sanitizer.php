<?php
/*
Plugin Name: Project URL Sanitizer
Plugin URI:
 ...
 * Sanitize title
 *
 * @param string $title the title of post/term/filename to be turned into slug
 *
 * @return sanitized title for slug
 */
function project_url_sanitizer_sanitize_title($title) {

  /**
   * Russian characters
   * @see https://wordpress.org/plugins/cyr3lat/
   */
  $iso9_table = array(
    'А' => 'A', 'Б' => 'B', 'В' => 'V', 'Г' => 'G', 'Ѓ' => 'G',
  ...
  /**
   * Arabic characters
   * @see https://wordpress.org/plugins/arabic-to-lat/
   */
  $iso8859 = array(
    "ا"=> "a","أ"=> "a","إ"=> "ie","آ"=> "aa",
    "ب"=> "b","ت"=> "t","ث"=> "th","ج"=> "j",
  ...
  $char_map = array_merge($iso9_table, $geo2lat, $iso8859);

  /**
   * Exceptions for locales
   * @see https://wordpress.org/plugins/cyr3lat/
   */
  $locale = get_locale();
  switch ($locale) {
  ...
  /**
   * Replace characters according to dictionary.
   * Dictionary can be extended with filter.
   */
  $title = strtr($title, apply_filters('project_url_sanitizer_table', $char_map));
  ...
  $slug_length = apply_filters('project_url_sanitizer_slug_length', 100);

  // build zh title from scratch
  $modified_title_zh = '';

  // save original title in case there is no Chinese to be found
  $original_title = $title;
  $has_chinese_chars = false;

  /**
   * Convert title to UTF-8.
   * This is probably critical for following byte by byte excecution.
   */
  if (function_exists('iconv') && get_bloginfo('charset') !== 'UTF-8') {
    $title = iconv(get_bloginfo('charset'), 'UTF-8', $title);
  }

  // substr long titles
  if ($slug_length > 0) {
    $title = substr($title, 0, $slug_length);
  }

  /**
   * Go through the title byte by byte.
   * Looking the individual bytes will
   * tell us if it's Chinese or not.
   *
   * @see https://wordpress.org/plugins/so-pinyin-slugs/
   */
  for ($i = 0; $i < strlen($title); $i++) {

    // extract 1 byte from title
    $byte = ord(substr($title, $i, 1));

    /**
     * If in range between 11100000 and 11101111 then it is a Chinese character in UTF-8. Yeah, really.
     *
     * @see https://wordpress.org/plugins/so-pinyin-slugs/
     */
    if ($byte >= 224 && $byte <= 239) {
      ...
      $full_char = substr($title, $i, 3);
      $i+=2;

      /**
       * Find spelling in the dictionary.
       * If it cannot find the character, it will be ignored.
       */
      foreach ($zh_to_lat as $pinyin => $val) {
        if (strpos($val, $full_char) !== false) {
          $modified_title_zh .= $pinyin;
          break;
        }
      }
      ...
      // append any character that is not Chinese
      $modified_title_zh .= chr($byte);

    }
  }

  // set or discard modified title
  if ($has_chinese_chars) {
    $title = $modified_title_zh;
  } else {
...
add_filter('sanitize_title',     'project_url_sanitizer_sanitize_title', 9);
add_filter('sanitize_file_name', 'project_url_sanitizer_sanitize_title');
